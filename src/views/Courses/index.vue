...
export default {
  setup() {
    ...
    return {courses, gotoNewRouter, openNewTab};
  },
};
</script>
<template>
  <div id="courses">
    <a class="card"  v-for="item in courses.data" :key="item.id" @click.left="gotoNewRouter(item.id)"
    ...
    <!-- <router-link class="card"  v-for="item in courses.data" :key="item.id" :to="`/Courses/${item.id}`">
      <img :src="item.photo" :alt="item.name" />
      <div class="content">
        <h1>{{item.name}}</h1>
        <div class="teacher-box">
